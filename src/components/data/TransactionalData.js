...
import {
  useDownloadTransactionalTemplate,
  useDownloadTransactionalMaster,
  useUploadTransactionalMaster,
  useViewTransactionalMaster,
} from "@/api/hooks/useTransactional";
...
const TransactionalData = ({ isActive = true }) => {
  ...
  const downloadTemplateMutation = useDownloadTransactionalTemplate();
  const downloadMasterMutation = useDownloadTransactionalMaster();
  const uploadMasterMutation = useUploadTransactionalMaster();
  const { data: masterData, isLoading } = useViewTransactionalMaster(
    {
      page,
      page_size: pageSize,
  ...
  const columnDefs = useMemo(
    () => [
      { field: "pay_code", headerName: "Pay Item", flex: 1, minWidth: 120 },
      { field: "pay_period_start", headerName: "Pay Period Start", flex: 1, minWidth: 140 },
      { field: "pay_period_end", headerName: "Pay Period End", flex: 1, minWidth: 140 },
      { field: "employee_id", headerName: "Employee ID", flex: 1, minWidth: 120 },
      { field: "hours_units", headerName: "Hours/Units", flex: 1, minWidth: 120 },
      { field: "rate", headerName: "Rate", flex: 1, minWidth: 100 },
      { field: "pay_amount", headerName: "Pay Amount", flex: 1, minWidth: 120 },
    ],
    []
  );
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const filename = getFilenameFromHeaders(response.headers, "Transactional_Template.xlsx");
  ...
  const handleDownloadCurrentMaster = async () => {
    try {
      ...
      const filename = getFilenameFromHeaders(response.headers, "Transactional_Master.xlsx");
      downloadFile(response.data, filename);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", gap: 2.5, position: "relative" }}>
        ...
        <Box sx={{ display: "flex", gap: 1.5, alignItems: "center" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
            <Button
              variant="outlined"
              startIcon={
                uploadMasterMutation.isPending || isParsing ? (
                  <CircularProgress size={16} />
                ) : (
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1.5, flexShrink: 0, flexWrap: "wrap", position: "relative", zIndex: 1 }}>
        ...
        <MasterDataActionTooltipButton
          tooltip={MODIFY_DATA_TOOLTIP}
          variant="contained"
          startIcon={<EditOutlinedIcon sx={{ fontSize: 16 }} />}
          onClick={handleOverrideClick}
          disabled={!selectedFile || uploadMasterMutation.isPending || isParsing}
          sx={{
            textTransform: "none",
            fontSize: 12,
        ...
        >
          Override Transactional Data
        </MasterDataActionTooltipButton>
        <MasterDataActionTooltipButton
          tooltip={ADD_DATA_TOOLTIP}
          variant="contained"
